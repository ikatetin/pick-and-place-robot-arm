...
private:
  ...
  void sliderCallback(const sensor_msgs::msg::JointState &msg) const
  {
    ...
    arm_command.joint_names = {"joint_1", "joint_2", "joint_3", "horizontal_arm_to_claw_support"};
    gripper_command.joint_names = {"joint_4"};

    trajectory_msgs::msg::JointTrajectoryPoint arm_goal, gripper_goal;
    arm_goal.positions.insert(arm_goal.positions.end(), msg.position.begin(), msg.position.begin() + 4);
    gripper_goal.positions.push_back(msg.position.at(4));
