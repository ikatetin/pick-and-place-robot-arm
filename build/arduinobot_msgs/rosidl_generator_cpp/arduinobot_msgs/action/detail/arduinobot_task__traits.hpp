  ...
  std::ostream & out)
{
  ...
  // member: arm_goal
  {
    if (msg.arm_goal.size() == 0) {
      out << "arm_goal: []";
    } else {
      out << "arm_goal: [";
      size_t pending_items = msg.arm_goal.size();
      for (auto item : msg.arm_goal) {
  ...
  // member: gripper_goal
  {
    if (msg.gripper_goal.size() == 0) {
      out << "gripper_goal: []";
    } else {
      out << "gripper_goal: [";
      size_t pending_items = msg.gripper_goal.size();
      for (auto item : msg.gripper_goal) {
        rosidl_generator_traits::value_to_yaml(item, out);
        if (--pending_items > 0) {
          out << ", ";
        }
